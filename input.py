"""
Shared input constants and legacy training/input utilities.

HM server uses only the constants in this file:
    PATCH_SIZE = 32
    GRID_SIZE = 8
    NUM_PATCHES_PER_IMAGE = 64

The training data readers, TFRecord helpers, and random patch sampling below are
legacy code paths and are not used by the HM server.
"""
...
# Legacy training/input pipeline below. HM server does not use these functions.
